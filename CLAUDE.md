...
1. **Data Sources**: Boris Chen's tiers/rankings (`utils/boris.ts`) are the
   ...
   Tier CSVs in `public/tiers/` are generated by the local fftiers repo and
   copied in with `npm run sync-tiers`. To refresh rankings, use the
   `sync-tiers` skill (`.claude/skills/sync-tiers/SKILL.md`) — it is also run
   by a daily 7 AM scheduled task.
